...
(() => {
  ...
  const PHASES = [
    ...
    },
    {
      id: 'A',
      status: 'done',
      release: { en: 'skill-map@0.5 · testkit@0.2', es: 'skill-map@0.5 · testkit@0.2' },
      title: { en: 'Deterministic core', es: 'Núcleo determinista' },
      sub: { en: 'Scan, model, query. No LLM.', es: 'Escanear, modelar, consultar. Sin LLM.' },
      brief: {
        en: 'Bytes hit disk. The spec became a working CLI: feed it any folder of agent files and it returns the full reference graph — collisions flagged, orphans listed, external deps mapped, all in milliseconds. The plugin runtime is real, not theoretical: drop a folder under .skill-map/plugins and the kernel picks it up. @skill-map/testkit ships alongside so authors can write their own extensions against a stable contract. A complete product, zero LLM calls.',
        es: 'Los bytes tocaron disco. La spec se volvió una CLI funcional: dale cualquier carpeta de archivos de agentes y te devuelve el grafo de referencias completo — colisiones marcadas, huérfanos listados, deps externas mapeadas, todo en milisegundos. El runtime de plugins es real, no teórico: dejas una carpeta bajo .skill-map/plugins y el kernel la levanta. @skill-map/testkit se publica junto con la CLI para que los autores escriban sus propias extensiones contra un contrato estable. Un producto completo, cero llamadas a LLM.',
      },
      list: 'steps',
      items: [
        ...
        { id: '6',  status: 'done',    title: { en: 'Config + onboarding',           es: 'Config + onboarding' },                    body: { en: '.skill-map/settings(.local).json · 6-layer config loader · sm config list/get/set/reset/show · .skill-mapignore wired into the scan walker · sm init scaffolding (DB + .gitignore append + first scan) · sm plugins enable/disable over config_plugins (DB > settings.json > default precedence) · frontmatter strict mode (--strict / scan.strict)', es: '.skill-map/settings(.local).json · loader de config en 6 capas · sm config list/get/set/reset/show · .skill-mapignore conectado al walker · scaffolding de sm init (DB + append a .gitignore + primer scan) · sm plugins enable/disable sobre config_plugins (precedencia DB > settings.json > default) · modo estricto de frontmatter (--strict / scan.strict)' } },
    ...
    },
    {
      ...
      sub: { en: 'Formatters, full web UI, single-binary release.', es: 'Formatters, UI web completa, release de un binario.' },
      brief: {
        ...
        es: 'Aquí el producto llega a 1.0. Formatters Mermaid y DOT para ops y CI, más providers para cubrir el ecosistema multi-host (Codex, Gemini, Copilot, genérico) y no sólo Claude, la UI web completa reemplaza el prototipo con un BFF Hono y eventos en vivo por WebSocket, y @skill-map/cli se distribuye como un único paquete npm con la UI empaquetada adentro. Un proceso, un puerto, un comando.',
      },
      list: 'steps',
      items: [
        ...
        { id: '13',  status: 'planned', title: { en: 'Multi-host adapters',             es: 'Adapters multi-host' },                     body: { en: 'Codex · Gemini · Copilot · generic provider (frontmatter-driven fallback) · per-host sm-<host>-* skill namespace · adapter conformance suite', es: 'Codex · Gemini · Copilot · provider genérico (fallback por frontmatter) · namespace sm-<host>-* por host · suite de conformance por adapter' } },
        { id: '14a', status: 'planned', title: { en: 'Web UI: BFF + transport',         es: 'UI web: BFF + transporte' },                body: { en: 'Hono BFF · WebSocket /ws · single-port mandate · Angular SPA + REST + WS under one listener · sm serve --port N',                  es: 'BFF Hono · WebSocket /ws · mandato de un solo puerto · SPA Angular + REST + WS bajo un solo listener · sm serve --port N' } },
        { id: '14b', status: 'planned', title: { en: 'Web UI: Flavor B vertical slice', es: 'UI web: slice vertical de Sabor B' },       body: { en: 'Inspector with enrichment + summaries + findings · command submit from UI · chokidar live updates over WS · MD body renderer pick', es: 'Inspector con enrichment + summaries + findings · submit de comandos desde la UI · updates en vivo de chokidar sobre WS · pick del renderer de MD' } },
        { id: '14c', status: 'planned', title: { en: 'Web UI: polish & budgets',        es: 'UI web: pulido y presupuestos' },           body: { en: 'URL-synced filter state · responsive scope decision · bundle budget compliance · dark mode tri-state evaluation · Foblex types reassessment', es: 'Estado de filtros sincronizado en URL · decisión de scope responsive · cumplimiento de bundle budget · evaluación de dark mode tri-state · revisión de tipos Foblex' } },
        { id: '15a', status: 'planned', title: { en: 'Distribution: single package',    es: 'Distribución: paquete único' },             body: { en: '@skill-map/cli with UI bundled · sm + skill-map binary aliases · sm ui sub-command · settings loader + runtime-settings schema',     es: '@skill-map/cli con UI incluida · alias de binarios sm + skill-map · sub-comando sm ui · loader de settings + schema runtime-settings' } },
