...
**Status**: pre-1.0, active development. The deterministic kernel, the CLI, and the bundled Web UI (`sm serve`) ship today. **Next**: Step 14.6 (bundle budget hard pass + Foblex strict types + dark-mode tri-state) and 14.7 (URL-synced filter state + responsive scope + production polish) close out `v0.6.0`. **After that**: wave 2 lands the optional LLM layer — job subsystem, probabilistic extensions, semantic verbs (`sm what`, `sm cluster-triggers`, `sm findings`) — shipping `v0.8.0`. Three public packages on npm: [`@skill-map/spec`](https://www.npmjs.com/package/@skill-map/spec), [`@skill-map/cli`](https://www.npmjs.com/package/@skill-map/cli), [`@skill-map/testkit`](https://www.npmjs.com/package/@skill-map/testkit). See [ROADMAP.md](./ROADMAP.md) for the execution plan.
