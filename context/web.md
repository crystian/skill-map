...
**JS guards must mirror CSS**: any `matchMedia('(max-width: Npx)')` in `web/modules/*.js` MUST match a CSS `@media (max-width: Npx)` rule, change one and the other in the same diff. There is none today: the hero card's former guards left with the hand-drawn SVG simulator they gated.

**The hero card is the real map, framed**: `#hero-graph` holds an `<iframe>` of `/demo/?embed=1&replay=main:demo-publish&theme=neon-blue` (the embed contract is `spec/provider-activity.md` §"Embedded replay": canvas only, looping replay, wheel left to the page, card click opens the full demo). `web/modules/hero-embed.js` sets the frame's `src` after the page `load` and fades it in over the poster (`web/img/hero-embed-poster.jpg`, a screenshot of that same graph; regenerate it when the demo fixture or the neon-blue theme changes) once the framed map has drawn its cards. The card stays on phones: the embed skips the app's desktop-only viewport guard, so do not reintroduce a `display: none` for it below 768px.
