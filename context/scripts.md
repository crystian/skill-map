...
**Everything else lives in its workspace**: typecheck, test, test:ci, test:coverage, lint, build, secondary dev modes, bundle-analyze, watch builds, clean. Invoke with `bun run --filter Y X` or by entering the workspace.
...
Root orchestrates with `bun run --filter '*' validate`. CI runs only this command — the individual steps in the `validate` job are removed as each workspace adopts its own `validate`.
...
```json
"prevalidate": "bun run install:browsers && bun run --filter @skill-map/web demo:build",
"validate": "bun run test:ci"
```
...
- ❌ **Root npm scripts that invoke a workspace's own `.js`.** If the `.js` belongs to the workspace, the npm script that invokes it lives in the workspace. Root only invokes via `bun run --filter Y X` (not via `node workspace/scripts/foo.js`).
...
- In-flight changes to landing, UI, schemas, fixtures, etc. are not visible at `skill-map.dev` until the next release. Use `bun run web:dev` locally to preview.
...
The hook hooks itself in automatically: the root `package.json` `prepare` script runs `git config core.hooksPath .githooks` every time someone runs `bun install`. No manual setup per contributor. The script is guarded with `[ -d .git ]` so `bun install --frozen-lockfile` inside Docker (where the `.git/` directory isn't copied into the build context) silently no-ops instead of failing on a missing `git` binary.
...
- **Move a script from root to a workspace**: update every reference (CI, docs, other scripts), run `bun run validate` before committing.
