...
- Provider: `claude`.
- Extractors: `frontmatter`, `slash`, `at-directive`.
- Rules: `trigger-collision`, `broken-ref`, `superseded`, `validate-all`.
- Formatter: `ascii`.

**Step 4** adds `external-url-counter` as the 4th extractor (drop-in proof of Step 2's acceptance test). It feeds the `scan_nodes.external_refs_count` denormalization that also lands in Step 4.
