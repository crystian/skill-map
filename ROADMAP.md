...
- `context/cli-reference.md` regenerated by the CLI workspace's `reference` script (delegates to `scripts/build-cli-reference.js`) from `sm help --format md`. Workspace scripts: `npm run reference --workspace=@skill-map/cli` writes, `npm run reference:check --workspace=@skill-map/cli` fails on drift. Current reference covers every verb — 290 lines, 6.5KB. ✅ landed.
