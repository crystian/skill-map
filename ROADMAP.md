...
●  9    Plugin author UX             plugin runtime · plugin migrations · @skill-map/testkit on npm · author guide + reference plugin
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE — Steps 0–9 + 14.1–14.5 complete · 14.6 (bundle budget + Foblex strict types + dark-mode tri-state) and 14.7 (URL-synced filter state + responsive scope + production polish) close v0.6.0.
  ────────────────────────────────────────────────────────────────────────
...
> ▶ **Completeness marker (2026-05-03)**: Steps **0a–9**, **14.1–14.4**, and **14.5 (a + b + c + d)** are complete. **Next**: Step **14.6** (bundle budget hard pass — flip the warning threshold from 6 KB to 500 KB, address the long-standing `node-card.css` budget warning, complete the Foblex strict-types pass, ship the dark-mode tri-state — auto/light/dark per system preference) followed by **14.7** (URL-synced filter state, responsive scope decision, production polish). Together they close `v0.6.0` (deterministic kernel + CLI + Web UI). Wave 2 (job subsystem + LLM layer) resumes after Step 14 closes and ships `v0.8.0`.
