...
**Status**: Steps **0a**, **0b**, **0c**, **1a**, and **1b** are **complete**. `@skill-map/spec` is published on npm; kernel shell + CLI skeleton boot and pass CI; the `ui/` workspace ships Flavor A end-to-end (list, inspector, graph, filters, simulated event flow, dark-mode toggle) against a mock collection; the kernel owns a fully migrated SQLite store behind `SqliteStorageAdapter` with the `sm db` command surface wired; the plugin loader discovers, validates, and dynamic-imports plugins against the published schemas via AJV and surfaces the three declared failure modes with precise diagnostics. Next step: **Step 1c — Orchestrator + CLI dispatcher + introspection**. The canonical completeness marker lives in §Execution plan below.
...
> ▶ **Completeness marker (2026-04-22)**: Steps **0a**, **0b**, **0c**, **1a**, and **1b** are **complete**. Step 1b shipped AJV-backed schema validation loaded from `@skill-map/spec`, hand-written kernel plugin types (`src/kernel/types/plugin.ts`), the `PluginLoader` adapter with all three declared failure modes (`invalid-manifest` / `incompatible-spec` / `load-error`) plus the `sm plugins list / show / doctor` surface, a side-fix to the extension-kind schemas that unblocked closed-content validation under allOf composition (`unevaluatedProperties: false` in place of the previous `additionalProperties: false` footgun), and an 8-case acceptance test codifying the ROADMAP criterion that a bogus plugin never takes the kernel down. Next step: **Step 1c — Orchestrator + CLI dispatcher + introspection**. Explicitly postponed by design: `preamble-bitwise-match` conformance case (deferred to Step 9, needs `sm job preview`), remaining tech picks (MD renderer, templating, pretty CLI, globbing, diff — each lands at the step that first needs it; MD renderer specifically flagged under Step 12 open picks), `sm db migrate --kernel-only` / `--plugin <id>` flags + plugin migrations with triple protection + typed-DTO emission from `@skill-map/spec` all deferred to Step 2 (see §DTO gap and §Step 1b deferral note), URL-synced filter state + responsive support + bundle-budget compliance + UI a11y baseline (Decision #74f) all deferred to Step 12.
