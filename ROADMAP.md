...
●  9    Plugin author UX             plugin runtime · plugin migrations · @skill-map/testkit on npm · author guide + reference plugin
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE (2026-05-02) — Phase A complete · **Step 14.4 fully closed** (a + b shipped) — UI-side consumer landed with `WsEventStreamService` (multicast RxJS observable backed by `WebSocket`, exponential reconnect 1s/2s/4s/8s/16s/30s capped at 10 attempts, EMPTY in demo mode), `RestDataSource.events()` wired through it, `CollectionLoaderService` reactive on `scan.completed` (with in-flight refresh coalescing), `EventLog` drawer FIFO ring of last 50 events with per-type digests + stream-error banner. UI tests 78 → 113 (+35). 14.5 (Inspector polish) opens next.
  ────────────────────────────────────────────────────────────────────────
