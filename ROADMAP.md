...
**Last updated**: 2026-04-27 (Step 5 closed: rename heuristic + history readers + orphans verbs + scan_meta + conformance fixtures + heuristic-on-every-scan fix + orphan persistence + history CLI polish + walkthrough-surfaced bundle (failure_reason inline, validator cache, yaml-canonicalize). 0 changesets pending — the 25-entry backlog (12 from Step 4 + 13 from Step 5) was drained via Version Packages PR #12, bumping `@skill-map/spec` → 0.6.0 and `@skill-map/cli` → 0.3.2). Changes land via `.changeset/*.md` and `spec/CHANGELOG.md` — this header stops paraphrasing them.
