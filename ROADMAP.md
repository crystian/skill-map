...
●  9    Plugin author UX             plugin runtime · plugin migrations · @skill-map/testkit on npm · author guide + reference plugin
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE (2026-05-02) — Phase A complete · Step 14.4.a landed (BFF half) — `WsBroadcaster` + chokidar `WatcherService` + composition-root lifecycle + `attachBroadcasterRoute` + `--no-watcher` / `--watcher-debounce-ms` flags + spec `### Server` `WebSocket protocol` subsection. Tests 832 → 854 (+22). 14.4.b still owes the UI-side `WsEventStreamService` consumer; once it lands 14.4 closes and 14.5 (Inspector polish) opens.
  ────────────────────────────────────────────────────────────────────────
