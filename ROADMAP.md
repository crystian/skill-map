...
●  9    Plugin author UX             plugin runtime · plugin migrations · @skill-map/testkit on npm · author guide + reference plugin
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE (2026-05-03) — Phase A complete · **Step 14.5.b landed** — `<sm-linked-nodes-panel>` standalone component renders extractor-detected outgoing+incoming links per inspector node (parallel listLinks fetch, scan.completed reactive refresh, manual refresh button, token guard). Relations card chips gained the dead-link verify icon (heuristic-then-server-confirm hybrid: `pi-question-circle` on heuristically-dead chips → click fires getNode → confirmed-dead shows `pi-times-circle`, alive flips to navigable). Template refactored to share a single `<ng-template #pathChip>` across the four relation buckets via `*ngTemplateOutlet`. UI tests 125 → 138 (+13: 10 panel + 3 verify-icon). 14.5.c (per-card refresh hooks) opens next.
  ────────────────────────────────────────────────────────────────────────
