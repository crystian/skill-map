...
**Last updated**: 2026-04-27 (Step 7 closed: 7.1 chokidar watcher + 7.2 detector conflict resolution + 7.3 `sm job prune` retention. 7.1: `sm watch` + `sm scan --watch` alias, `scan.watch.debounceMs` config key, debounced batches re-running incremental scans through the existing `runScanWithRenames` pipeline. 7.2: new built-in rule `link-conflict` that emits one `warn` per `(source, target)` pair where detectors disagree on `kind`; `sm show` pretty output now groups identical-shape links by `(endpoint, kind, normalizedTrigger)` and lists the union of detector ids in a `sources:` field with `(N, M unique)` header counts. Storage unchanged (Decision #90 stands — raw rows per detector). 7.3: `sm job prune` real implementation (replaces the stub) — applies `jobs.retention.{completed,failed}` policy, deletes terminal jobs older than the cutoff, unlinks their MD files, optional `--orphan-files` mode for FS-only cleanup, `--dry-run` + `--json` supported; `state_executions` left intact (append-only through v1.0). Trigger normalization (originally listed under Step 7) closed implicitly — cabled into every detector at Steps 3–4 with `src/kernel/trigger-normalize.test.ts` covering worked examples; no dedicated sub-step. UI inspector aggregation deferred to Step 13 because Flavor A renders `metadata.{related,requires,...}` chips directly from frontmatter, not from `scan_links` rows. 341 of 341 tests pass (303 → 341 = +38 across 7.1 + 7.2 + 7.3). 3 changesets pending in `.changeset/`: `step-7-1-chokidar-watcher.md` (minor for spec + cli — new verb + new config key), `step-7-2-link-conflict.md` (patch for spec, minor for cli — new rule + new presentation; JSON contract unchanged), and `step-7-3-job-prune.md` (minor for cli — verb activated from stub). Next step: **Step 8 — Diff + export** (`sm scan --compare-with`, `sm export`, `sm graph`). Changes land via `.changeset/*.md` and `spec/CHANGELOG.md` — this header stops paraphrasing them.
