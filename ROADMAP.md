...
**Status**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, **2**, **3**, and **4** are **complete**. `@skill-map/spec` is published on npm; the `ui/` workspace ships Flavor A. Step 4 shipped the scan end-to-end pipeline across 11 sub-steps: 4.1 persistence, 4.2 token counts (`js-tiktoken` cl100k_base + `--no-tokens`), 4.3 `external-url-counter` detector (drop-in proof, partitioned out of `scan_links`), 4.4 incremental `--changed` + `-n / --dry-run`, 4.5 `sm list / show / check` readers, 4.6 self-scan acceptance test + 500-MD performance benchmark (~1037 ms vs 2000 ms budget), 4.7 runtime↔spec ScanResult reconciliation (runtime caught up — `scannedAt` int, `scope`, `adapters`, `scannedBy`, `stats.filesWalked / filesSkipped`), 4.8 first bug bundle (exit-code spec compliance, `sm show` external refs line, supersedes-inversion link survival under `--changed`), 4.9 second bug bundle (trigger-collision advertiser detection from `frontmatter.name`, WAL checkpoint after persist so external tools see fresh state), 4.10 scenario coverage (10 new regression tests for hash discrimination, external-refs lifecycle, deletion-driven broken-ref re-evaluation, replace-all ID rotation, `--no-tokens` CLI handler, `--changed --no-built-ins` rejection, trigger-collision under `--changed`), 4.11 empty-scan guard against typo-traps (root validation in `runScan` + `--allow-empty` opt-in for explicit wipes — closes the bug where `sm scan -- --dry-run` parsed `--dry-run` as a positional root and silently wiped a populated DB). Bug fixed in `defaultMigrationsDir()` (latent regression in dist bundle layout). 151 of 151 tests pass (88 → 151 = +63 across Step 4). Next step: **Step 5 — History + orphan reconciliation**. The canonical completeness marker lives in §Execution plan below.
