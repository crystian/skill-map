...
> ▶ **Completeness marker (2026-04-22)**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, and **2** are **complete**. Step 2 shipped the runtime contracts for the six extension kinds, the `claude` adapter, three detectors (frontmatter / slash / at-directive), three rules (trigger-collision / broken-ref / superseded), the `ascii` renderer, the `validate-all` audit, the built-ins registry, the orchestrator wire-up, and an end-to-end scan test against a fixture collection — 88 of 88 tests pass. `@skill-map/spec` got a patch fix along the way (extension-kind schemas switched from `additionalProperties: false` to `unevaluatedProperties: false` so closed-content survives allOf composition). Next step: **Step 3 — Scan end-to-end** (`sm scan --changed`, denormalised `external_refs_count`, `external-url-counter` as the drop-in litmus proof). Explicitly postponed by design: `preamble-bitwise-match` conformance case (deferred to Step 9, needs `sm job preview`), remaining tech picks (MD renderer, templating, pretty CLI, globbing, diff — each lands at the step that first needs it; MD renderer specifically flagged under Step 12 open picks), `sm db migrate --kernel-only` / `--plugin <id>` flags + plugin migrations with triple protection (pending an SQL parser/rewriter, likely Step 2.5 or Step 3), typed-DTO emission from `@skill-map/spec` pragmatically resolved via hand-curated mirrors with AJV for runtime safety (closing from spec waits for a third-party consumer outside the repo), URL-synced filter state + responsive support + bundle-budget compliance + UI a11y baseline (Decision #74f) all deferred to Step 12.
...
Acceptance: `sm help` covers every verb in the spec; `docs/cli-reference.md` is byte-equal to `sm help --format md` output and `npm run cli:check` blocks drift; `kernel-empty-boot` passes via the real orchestrator. 36 of 36 tests pass (32 prior + 4 new covering scan event emission, empty-registry orchestrator iteration, and InMemoryProgressEmitter subscribe/unsubscribe).
