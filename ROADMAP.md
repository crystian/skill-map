...
**Status**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, **2**, **3**, **4**, **5**, **6**, **7**, and **8** are **complete**. `@skill-map/spec` is published on npm; the `ui/` workspace ships Flavor A. Most recently, Step 7 closed the deterministic-half robustness pass (`sm watch` / `sm scan --watch`, `link-conflict` rule, `sm job prune`); a follow-up spec edit lifted **execution modes (`deterministic` / `probabilistic`) to a first-class architectural property** spanning every analytical extension; and Step 8 shipped the diff + export verbs (`sm graph`, `sm scan --compare-with`, `sm export`) — every spec'd CLI verb has a real implementation now except the Step 9 plugin author surface, the Step 10 / 11 probabilistic queue, and the Step 14 web server. 389 of 389 tests pass. Next step: **Step 9 — Plugin author UX**. The canonical completeness marker lives in §Execution plan below; per-sub-step prose for closed steps lives in changesets and the history blocks beneath the marker.
