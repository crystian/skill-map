...
**Last updated**: 2026-04-27 (Step 5 closed: rename heuristic + history readers + orphans verbs + scan_meta + conformance fixtures + heuristic-on-every-scan fix + orphan persistence + history CLI polish + walkthrough-surfaced bundle (failure_reason inline, validator cache, yaml-canonicalize). 25 changesets pending in `.changeset/` — 12 from Step 4 + 13 from Step 5; the Step 4 batch was never drained, so the next Version Packages PR rolls both up). Changes land via `.changeset/*.md` and `spec/CHANGELOG.md` — this header stops paraphrasing them.
...
**Status**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, **2**, **3**, **4**, and **5** are **complete**. `@skill-map/spec` is published on npm; the `ui/` workspace ships Flavor A. Step 5 shipped history + orphan reconciliation across 10 sub-steps: 5.1 `scan_meta` (closes 4.7 follow-up — real `scope/roots/scannedBy/stats` instead of synthetic), 5.2 storage helpers (`insertExecution`, `listExecutions`, `aggregateHistoryStats`, `migrateNodeFks` covering simple column / json1 array / composite-PK delete+insert), 5.3 `sm history` CLI, 5.4 `sm history stats` CLI with self-validation against `history-stats.schema.json`, 5.5 auto-rename heuristic in scan path (high body / medium frontmatter / ambiguous N:1 / orphan, tx-atomic FK migration), 5.6 `sm orphans` + `reconcile` + `undo-rename` verbs (extracted shared `confirm()` helper), 5.7 conformance fixtures (`rename-high`, `orphan-detection`) + spec extension `setup.priorScans`, 5.8 fire heuristic on every `sm scan` (decoupled `priorSnapshot` data from `enableCache` behaviour), 5.9 orphan persistence across scans (`findStrandedStateOrphans` sweep on every persist so `sm orphans reconcile` stays usable), 5.10 history CLI polish (table column widths + `elapsedMs` accuracy in JSON). 213 of 213 tests pass (151 → 213 = +62 across Step 5, including the three walkthrough-surfaced sub-steps 5.11–5.13). 25 changesets pending in `.changeset/` — 12 carried over from Step 4 (never drained) + 13 from Step 5; the next Version Packages PR rolls both up. Next step: **Step 6 — Config + onboarding**. The canonical completeness marker lives in §Execution plan below.
...
> ▶ **Completeness marker (2026-04-27)**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, **2**, **3**, **4**, and **5** are **complete**. Step 5 shipped history + orphan reconciliation across ten sub-steps (5.1 — `scan_meta` table closes the 4.7 follow-up by persisting `scope/roots/scannedBy/stats`; 5.2 — storage helpers `insertExecution`, `listExecutions`, `aggregateHistoryStats`, `migrateNodeFks` with three FK shapes; 5.3 — `sm history` CLI with filters and `done in <…>` per cli-contract §Elapsed time; 5.4 — `sm history stats` with UTC bucketing, top-N nodes, perFailureReason always-six-keys, schema self-validation; 5.5 — auto-rename heuristic in scan path with high body match → silent migration, medium frontmatter 1:1 → `auto-rename-medium` issue + migration, ambiguous N:1 → `auto-rename-ambiguous` no migration, residual → `orphan`; tx-atomic FK migration before scan zone replace-all; 5.6 — `sm orphans` + `reconcile` + `undo-rename` verbs sharing `confirm()` helper; 5.7 — conformance fixtures `rename-high` and `orphan-detection` + spec-additive minor extension `setup.priorScans` for staging; 5.8 — fire heuristic on every `sm scan` not just `--changed`, decoupled `priorSnapshot` (data) from `enableCache` (perf); 5.9 — orphan persistence across scans via `findStrandedStateOrphans` sweep on every persist so `sm orphans reconcile` stays usable as long as state has stranded refs; 5.10 — history CLI polish for table column collapse and `elapsedMs` accuracy in JSON). Step 5 closed: 151 → **213 of 213 tests pass** (+62 across all thirteen Step 5 changesets, sub-steps 5.1–5.10 plus walkthrough follow-ups 5.11–5.13). **25 changesets** pending in `.changeset/` — 12 carried over from Step 4 (never drained — the `cli-reference-path-move` cleanup plus the eleven Step 4 sub-step entries) and 13 from Step 5; the next Version Packages PR rolls both batches up. Next step: **Step 6 — Config + onboarding**. Step 4 was previously closed at 151 tests; details of Step 4's eleven sub-steps live in `spec/CHANGELOG.md` and the changeset history. Explicitly postponed by design (carried over from Step 4): `preamble-bitwise-match` conformance case (deferred to Step 10, needs `sm job preview`), remaining tech picks (MD renderer, templating, pretty CLI, globbing, diff — each lands at the step that first needs it; MD renderer specifically flagged under Step 13 open picks), `sm db migrate --kernel-only` / `--plugin <id>` flags + plugin migrations with triple protection (deferred to Step 9), URL-synced filter state + bundle-budget full compliance + UI a11y baseline (Decision #74f) all deferred to Step 13. Walkthrough-surfaced follow-ups all closed in this Step (5.11 — `sm history` human shows `failure_reason` inline, 5.12 — `loadSchemaValidators` cached at module level, 5.13 — frontmatter hash computed over canonical YAML so formatter passes don't break rename detection). Carried over from Step 4: URL-extraction regex truncates parens (Wikipedia/MDN URLs short-changed) and reference-style markdown links match only on the definition line — both are intentional trade-offs flagged for a future detector polish pass; `SqliteStorageAdapter` can't open `:memory:` because `init()` opens two separate `DatabaseSync` connections (tests use `mkdtempSync` file paths instead). The original Step 4 sub-step prose is preserved in the canonical changesets and prior README revisions; condensed here so the marker stays scannable.
...
> Step 5 closed: 151 → **213 of 213 tests pass** (+62 across Step 5). 25 changesets pending in `.changeset/` — 12 from Step 4 (never drained) + 13 from Step 5; the next Version Packages PR rolls both up.
