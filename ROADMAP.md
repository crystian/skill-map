...
> ▶ **Completeness marker (2026-04-22)**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, and **2** are **complete**. Post-0c review pass done (same date). Step 2 shipped the runtime contracts for the six extension kinds, the `claude` adapter, three detectors (frontmatter / slash / at-directive), three rules (trigger-collision / broken-ref / superseded), the `ascii` renderer, the `validate-all` audit, the built-ins registry, the orchestrator wire-up, and an end-to-end scan test against a fixture collection — 88 of 88 tests pass. Step 0c UI prototype completed to 100% scope (link-count badges, priority column, mocked summary, issue filter, det/prob buttons added). Post-0c review resolved: Decision #72 revised (Angular tracks latest, no major pin), DTO gap scheduled for codegen at Step 4–5, plugin migrations deferred to Step 9, new Decision #89 (plugin API stable at v1.0), new Decision #90 (link conflicts: both rows coexist, no merge). Next step: **Step 3 — UI design refinement** (node cards, connections, inspector layout, responsive, dark mode, bundle budget). Explicitly postponed by design: `preamble-bitwise-match` conformance case (deferred to Step 10, needs `sm job preview`), remaining tech picks (MD renderer, templating, pretty CLI, globbing, diff — each lands at the step that first needs it; MD renderer specifically flagged under Step 13 open picks), `sm db migrate --kernel-only` / `--plugin <id>` flags + plugin migrations with triple protection (deferred to Step 9), URL-synced filter state + responsive support + bundle-budget compliance + UI a11y baseline (Decision #74f) all deferred to Step 13.
...
- Node card redesign: determine what information each kind (skill, agent, command, hook, note) shows on the graph card and in the list row.
- Connection styling: edge thickness, labels, directionality cues, color semantics.
- Inspector layout: card ordering, information hierarchy, visual density.
- Responsive baseline: usable at 1024px+.
- Dark mode parity: both themes look intentional.
- Empty / error / loading states polished.
- Bundle budget: investigate the 79 kB overshoot (PrimeNG theme).
