...
**Status**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, **2**, **3**, **4**, **5**, **6**, and **7** are **complete**. `@skill-map/spec` is published on npm; the `ui/` workspace ships Flavor A. Most recently, Step 7 closed the deterministic-half robustness pass (`sm watch` / `sm scan --watch`, `link-conflict` rule, `sm job prune`) and a follow-up spec edit lifted **execution modes (`deterministic` / `probabilistic`) to a first-class architectural property** spanning every analytical extension — defining a per-kind capability matrix (Detector / Rule / Action dual-mode by manifest declaration; Audit dual-mode with mode derived from `composes[]`; Adapter / Renderer deterministic-only) and the `RunnerPort` injection contract. 341 of 341 tests pass. Next step: **Step 8 — Diff + export**. The canonical completeness marker lives in §Execution plan below; per-sub-step prose for closed steps lives in changesets and the history blocks beneath the marker.
...
✅ 6   Project config          .skill-map/settings(.local).json, .skill-mapignore, sm init,
  ...
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE (2026-04-27) — complete through 6 · next: 7
  ────────────────────────────────────────────────────────────────────────
...
═══════════════════════════════════════════════════════════════════════════
  ...
  13  More adapters            Codex, Gemini, Copilot, generic
  14  Web UI full              Flavor B vertical slice, graph + inspector,
                               WebSocket live events, command submit from UI
  15  Distribution polish      releases, docs site, marketplace, sm doctor,
...
═══════════════════════════════════════════════════════════════════════════
  16+  Write-back              edit / create / refactor from UI
  17+  Test harness            dry-run / real / subprocess
  18+  Richer workflows        Node-pipe, JSON declarative, DAG
  19+  Additional lenses       Obsidian-vault, docs-site
...
> ▶ **Completeness marker (2026-04-28)**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, **2**, **3**, **4**, **5**, **6**, and **7** are **complete**. Step 7 — robustness — shipped across three sub-steps (7.1 — chokidar watcher: `sm watch` as a first-class verb plus `sm scan --watch` alias, new `scan.watch.debounceMs` config key with sensible default, debounced batch flush re-running incremental scans through the existing `runScanWithRenames` pipeline so no new persistence path was introduced; 7.2 — detector conflict resolution: new built-in rule `link-conflict` at `src/extensions/rules/link-conflict/index.ts` emits one `warn` per `(source, target)` pair where two or more detectors disagree on `kind` — Decision #90 stands and `scan_links` keeps raw rows per detector, with the rule reading the merged graph after detection; `sm show` pretty output now groups identical-shape links by `(endpoint, kind, normalizedTrigger)` and lists the union of detector ids in a `sources:` field with `(N, M unique)` header counts so inspector-style reading scans cleanly even when three detectors fire on the same target; trigger normalization — originally a separate Step 7 line item — closed implicitly because every detector at Steps 3–4 was already cabled through `src/kernel/trigger-normalize.ts` with `src/kernel/trigger-normalize.test.ts` covering the worked examples; 7.3 — `sm job prune` real implementation replacing the stub: reads `jobs.retention.{completed,failed}` from layered config, deletes `state_jobs` rows in the matching terminal status with `finished_at < Date.now() - policySeconds * 1000`, unlinks the matching MD files in `.skill-map/jobs/`, optional `--orphan-files` mode runs a second pass that scans the directory and unlinks MD files no `state_jobs.file_path` references, `--dry-run` reports without touching DB or FS, `--json` emits `{ dryRun, retention: { completed: { policySeconds, deleted, files }, failed: {...} }, orphanFiles }`; `state_executions` is intentionally **not** touched — append-only through `v1.0` per `spec/db-schema.md`; pruning runs only on explicit invocation, never as implicit GC during normal verbs). UI inspector aggregation deferred to Step 14 because Flavor A renders `metadata.{related,requires,...}` chips directly from frontmatter, not from `scan_links` rows. Step 7 closed: 303 → **341 of 341 tests pass** (+38 across 7.1 + 7.2 + 7.3). Immediately after Step 7, a follow-up spec edit landed: **execution modes lifted to a first-class architectural property** (`@skill-map/spec` major bump). Previously the dual-mode capability was Action-only wording scattered through schemas; now `architecture.md` carries a top-level §Execution modes section before §Extension kinds defining the per-kind capability matrix (Detector / Rule / Action dual-mode by manifest declaration; Audit dual-mode with mode **derived** from `composes[]` at load time; Adapter / Renderer deterministic-only because they sit at the system boundaries), the runtime separation (`deterministic` runs in `sm scan` / `sm check` / `sm audit`; `probabilistic` dispatches as `sm job submit <kind>:<id>` and the kernel rejects probabilistic extensions that try to register scan-time hooks), and the `RunnerPort` injection contract for probabilistic extensions. Schemas updated: Detector / Rule gain optional `mode` (default `deterministic`, so all existing detectors / rules stay valid without an update); Action enum renamed (`local` → `deterministic`, `invocation-template` → `probabilistic` — the only breaking change, no third-party Actions exist); Audit forbids declaring `mode` directly (load-time error) and the kernel derives it from `composes[]`; Adapter / Renderer state explicitly that `mode` MUST NOT appear and recommend using a probabilistic Detector for LLM-assisted classification. The §Extension kinds table now states each row's mode posture explicitly. Bug fix collected by the same edit: `action.schema.json` previously declared `allOf` twice at the root (lines 6–8 and 71–80 in the prior shape); the second silently overrode the first, dropping the `$ref: base.schema.json` composition — both blocks merged into a single `allOf` so the action schema actually composes the base shape. The two READMEs gained a positioning section ("Two execution modes — the meta-architecture" / "Dos modos de ejecución — la meta-arquitectura"); the glossary table at the top of this document mirrors the spec table. Conformance case `extension-mode-derivation` filed under `spec/conformance/coverage.md` row C, deferred to Step 10 (needs the job subsystem to verify dispatch routing for the four sub-cases — deterministic-only composition, mixed composition, direct `mode` declaration, dangling reference). Threading `mode: 'deterministic'` explicitly through `src/extensions/built-ins.ts` (and adding the field to `IDetector` / `IRule` interfaces) is filed as a follow-up; today every built-in relies on the schema default and runtime behaviour is unchanged. Test count unchanged by the spec edit (341 → 341 — pure docs + schema). **4 changesets** pending in `.changeset/`: `step-7-1-chokidar-watcher.md` (minor for spec + cli — new verb + new config key), `step-7-2-link-conflict.md` (patch for spec, minor for cli — new rule + new presentation; JSON contract unchanged), `step-7-3-job-prune.md` (minor for cli — verb activated from stub), and `execution-modes.md` (**major** for spec — Action enum rename is breaking). Next step: **Step 8 — Diff + export** (`sm scan --compare-with`, `sm export`, `sm graph`). Explicitly postponed by design (carried over from earlier steps): `preamble-bitwise-match` conformance case (deferred to Step 10, needs `sm job preview`), `extension-mode-derivation` conformance case (deferred to Step 10, needs the job subsystem), explicit `mode` threading through built-ins + `IDetector` / `IRule` (follow-up alongside the first probabilistic extension at Step 10), remaining tech picks (MD renderer, templating, pretty CLI, globbing, diff — each lands at the step that first needs it; MD renderer specifically flagged under Step 14 open picks), `sm db migrate --kernel-only` / `--plugin <id>` flags + plugin migrations with triple protection (deferred to Step 9), URL-synced filter state + bundle-budget full compliance + UI a11y baseline (Decision #74f) all deferred to Step 14. Carried over from prior steps: URL-extraction regex truncates parens (Wikipedia/MDN URLs short-changed) and reference-style markdown links match only on the definition line — both are intentional trade-offs flagged for a future detector polish pass; `SqliteStorageAdapter` can't open `:memory:` because `init()` opens two separate `DatabaseSync` connections (tests use `mkdtempSync` file paths instead). Step 6 sub-step prose is preserved in its dedicated history block below; Step 5 prose lives in the canonical changesets and prior README revisions.
...
- `context/cli-reference.md` regenerated by `scripts/build-cli-reference.mjs` from `sm help --format md`. Root scripts: `npm run cli:reference` writes, `npm run cli:check` fails on drift. Current reference covers every verb — 290 lines, 6.5KB. ✅ landed.
...
- **8.3 — `sm export <query> --format <json|md|mermaid>`** — pending. Filtered export over the persisted graph; query syntax implementation-defined pre-1.0 (planned: `kind=skill[,agent]`, `has=issues`, `path=foo/*`). `mermaid` defers to Step 12.
...
**Open picks deferred from Step 0c review pass** (must land by Step 14 ship):
...
- **Bundle budget compliance** — prod budgets in `angular.json` stay armed (500 KB warn / 1 MB error); Step 14 includes the tree-shake + route-splitting pass that brings Flavor B under those.
- **Dark mode**: Step 0c ships a working toggle; Step 14 decides if it grows into a system-preference-aware tri-state and gains palette-level customisation.
