...
●  9    Plugin author UX             plugin runtime · plugin migrations · @skill-map/testkit on npm · author guide + reference plugin
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE (2026-05-02) — Phase A complete · Step 14.3 fully closed (a + b) — 14.3.a closed UI live-mode vertical slice (DataSourcePort + REST adapter + CollectionLoader migration + URL-sync + Inspector empty states + dead-file removal); 14.3.b closed StaticDataSource + markdown-it + DOMPurify renderer (lazy-loaded) + lazy graph chunk + `ui/mock-collection/` → `ui/fixtures/demo-scope/` relocation + demo build pipeline (`scripts/build-demo-dataset.js` + `scripts/patch-demo-mode.js` + root `npm run demo:build`) + dismissible demo banner. Next: 14.4 (WS broadcaster + EventStreamPort)
  ────────────────────────────────────────────────────────────────────────
