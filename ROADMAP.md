...
**Status**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, and **1c** are **complete** — every Step 1 sub-step done. `@skill-map/spec` is published on npm; the `ui/` workspace ships Flavor A end-to-end against a mock collection; the kernel owns a fully migrated SQLite store with the full `sm db` surface, a plugin loader with three failure modes against AJV-validated schemas, a real scan orchestrator that emits `scan.started` / `scan.completed` through a ProgressEmitterPort, every CLI verb from `cli-contract.md` registered (35 as stubs that print "not-implemented" and exit 2, the rest real), and auto-generated `docs/cli-reference.md` with CI drift check. The `kernel-empty-boot` conformance case passes end-to-end through the real orchestrator. Next step: **Step 2 — First extension instances**. The canonical completeness marker lives in §Execution plan below.
...
> ▶ **Completeness marker (2026-04-22)**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, and **1c** are **complete** — every Step 1 sub-step is done. Step 1c shipped the real scan orchestrator with ProgressEmitterPort wiring (`scan.started` / `scan.completed` emitted through a default `InMemoryProgressEmitter`), 35 typed Clipanion stubs covering every verb in `cli-contract.md` that doesn't yet have a real implementation, a `sm help` command with `human | md | json` formats backed by `cli.definitions()` introspection, and an auto-generator for `docs/cli-reference.md` with a CI-friendly drift check (`npm run cli:check`). `kernel-empty-boot` conformance passes through the real orchestrator. Next step: **Step 2 — First extension instances**. Explicitly postponed by design: `preamble-bitwise-match` conformance case (deferred to Step 9, needs `sm job preview`), remaining tech picks (MD renderer, templating, pretty CLI, globbing, diff — each lands at the step that first needs it; MD renderer specifically flagged under Step 12 open picks), `sm db migrate --kernel-only` / `--plugin <id>` flags + plugin migrations with triple protection + typed-DTO emission from `@skill-map/spec` all deferred to Step 2 (see §DTO gap and §Step 1b deferral note), URL-synced filter state + responsive support + bundle-budget compliance + UI a11y baseline (Decision #74f) all deferred to Step 12.
