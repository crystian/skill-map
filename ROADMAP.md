...
**Status**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, **2**, **3**, **4**, **5**, **6**, **7**, **8**, and **9** are **complete**. `@skill-map/spec` is published on npm; the `ui/` workspace ships Flavor A; `@skill-map/testkit` joined as the third public package. Step 9 — Plugin author UX — shipped four sub-steps: 9.1 (plugin runtime wiring — drop-in plugins now feed `sm scan` / `sm graph`), 9.2 (plugin migrations + three-layer prefix-enforcement rule + `sm db migrate --kernel-only` / `--plugin <id>`), 9.3 (`@skill-map/testkit` separate workspace with builders / context factories / KV + runner fakes / run helpers), and 9.4 (plugin author guide at `spec/plugin-author-guide.md`, reference plugin at `examples/hello-world/`, diagnostics polish on `PluginLoader.reason` strings). A subsequent **pre-1.0 audit review pass** (`cli-architect` REVIEW mode, 28/28 findings closed) consolidated kernel + CLI hygiene (composer split of `runScanInternal`, `withSqlite` helper across 20 read-side call sites, `ExitCode` enum, plugin-loader timeout, unified `--dry-run` contract on `init` / `db reset` / `db restore` codified in a new spec §Dry-run, `compare-with` promoted to a sub-verb, `*.texts.ts` template extraction with the new `tx` helper, migrations consolidated). 512 of 512 tests pass (482 cli + 30 testkit). **Next step (post-pivot 2026-05-02)**: **Step 14 — Full Web UI** (sub-stepped 14.1–14.7, deterministic UI vertical slice + demo build pipeline; ships v0.6.0). Step 10 (job subsystem) paused with Phase 0 (`IAction` runtime contract) landed and dormant; Phases A–G resume after Step 14 closes and ship v0.8.0 (per Decision #118). The canonical completeness marker lives in §Execution plan below; per-sub-step prose for closed steps lives in changesets and the history blocks beneath the marker.
...
●  9    Plugin author UX             plugin runtime · plugin migrations · @skill-map/testkit on npm · author guide + reference plugin
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE (2026-05-03) — Phase A complete · **Step 14.5 fully closed** (a + b + c shipped) — Inspector polish done. Body card renders markdown via the lazy MarkdownRenderer with header refresh button; `<sm-linked-nodes-panel>` renders extractor-detected outgoing+incoming links with its own refresh; Relations chips have the dead-link verify icon hybrid; template refactored via `<ng-template #pathChip>` + `*ngTemplateOutlet` to dedupe the four relation buckets. UI tests 113 → 142 (+29 across 14.5 a + b + c). CLI tests 854 → 868 (+14 in 14.5.a). 14.6 (bundle budget hard pass + Foblex strict types + dark-mode tri-state) opens next.
  ────────────────────────────────────────────────────────────────────────
...
> ▶ **Completeness marker (2026-05-03)**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, **2**, **3**, **4**, **5**, **6**, **7**, **8**, **9**, **14.1**, **14.2**, **14.3** (a + b), **14.4** (a + b), and **14.5** (a + b + c) are **complete**. **Next**: Step **14.6** (bundle budget hard pass — flip the warning threshold from 6 KB to 500 KB, address the long-standing `node-card.css` budget warning that's been registered since 14.3.b, complete the Foblex strict-types pass deferred from Step 0c, ship the dark-mode tri-state — auto/light/dark per system preference, deferred from Step 14.3). **Step 14.1 (`sm serve` + Hono BFF skeleton, 2026-05-02)** — first sub-step of the Web UI work landed. New `src/server/` workspace ships the Hono BFF as a peer of `src/cli/` (driver, not a kernel port impl): `index.ts` (`createServer(opts) → ServerHandle` — wires the Hono app, the `WebSocketServer({ noServer: true })`, and `@hono/node-server`'s `serve({ websocket: { server: wss } })` into one listener), `app.ts` (Hono app construction with single-port wiring: `GET /api/health` real, `ALL /api/*` 404 stubs, `GET /ws` via the injected `attachWs` registrar, static + SPA fallback; global `app.onError` formats every uncaught throw into the error envelope), `static.ts` (wraps `@hono/node-server`'s `serveStatic` middleware with the SPA-fallback layer — `serveStatic` `next()`s on miss, which is exactly the seam we hook into; absolute root paths work on POSIX in node-server@2.0.1, runtime-verified, the `.d.ts` warning is stale per upstream issue honojs/node-server#187), `ws.ts` (`noopWebSocketRoute(app)` registers `GET /ws` via the official `upgradeWebSocket` re-exported from `@hono/node-server@2.x`; the 14.1 handler closes the connection in `onOpen` with code 1000 + reason `'no broadcaster yet'`), `options.ts` (`IServerOptions` + `validateServerOptions`), `paths.ts` (`resolveDefaultUiDist` walks upwards looking for `ui/dist/browser/index.html`), `health.ts` (`buildHealth` synchronous; `resolveSpecVersion` async at boot), `i18n/server.texts.ts`. Kernel-boundary rules respected: no `console.*` in `server/` (uses i18n catalog + sanitization helpers); no direct `process.cwd()` / `homedir()` (the verb threads `defaultRuntimeContext()` in). `ServeCommand` extracted from `cli/commands/stubs.ts` to dedicated `cli/commands/serve.ts` extending `SmCommand` with `protected emitElapsed = false` (long-running daemon — mirrors `sm watch`). Flag surface matches spec exactly: `--port` (default `4242`), `--host` (default `127.0.0.1` per Decision #119), `--scope project|global` (alias for `-g/--global`), `--db <path>`, `--no-built-ins`, `--no-plugins`, `--open` / `--no-open`, `--dev-cors`, `--ui-dist <path>` (hidden). Loopback-only enforcement: `--dev-cors` + non-loopback `--host` rejects with exit 2. Boot resilience: `/api/health` reports `db: 'missing'` when the project DB is absent (per Decision #119 / pivot rule); `--db <missing>` is the only DB-related path that exits 5 (NotFound). Pinned dependencies (exact, per AGENTS.md): `hono@4.12.16`, `@hono/node-server@2.0.1`, `ws@8.20.0` (deps); `@types/ws@8.18.1` (dev) — added to `src/package.json` only. Dev tooling: root + workspace `dev:serve` script wraps `tsx --watch sm serve`, freeing port 4242 via `scripts/dev-serve.js` before booting (refuses to kill anything whose cmdline doesn't match `tsx|cli/entry|sm serve`). Spec impact (additive minor): `cli-contract.md` `sm serve` row replaced with the full 14.1 flag set; new `#### Server` subsection documents the single-port mandate, the boot-with-missing-DB resilience contract, the v14.1 endpoint surface, the structured error envelope, and the flag table — marked `*(Stability: experimental — locks at v0.6.0.)*`. `spec/index.json` regenerated (40 files hashed); `npm run spec:check` green. **15 new tests** across `test/server-boot.test.ts` (7), `test/server-flags.test.ts` (6), `test/server-db-missing.test.ts` (2); test totals 749 → **764 of 764 pass**. 1 changeset pending: `.changeset/step-14-1-bff-skeleton.md` (minor for spec + minor for cli). **Decision flagged for record**: WebSocket support uses `@hono/node-server@2.x`'s built-in `upgradeWebSocket` plus the canonical `ws@8.20.0` Node WebSocket library, per the official README pattern. The previously-published `@hono/node-ws` adapter was deprecated when node-server@2.0 absorbed WebSocket support natively (PR honojs/node-server#328). The 14.4 broadcaster swaps `noopWebSocketRoute` for its own one-line registrar — no API churn between 14.1 and 14.4. **Earlier prose (Pre-1.0 audit review pass, 2026-04-29)** — `cli-architect` REVIEW mode produced a 28-item Critical / High / Medium / Low / Nit catalog; all 28 closed in this pass. Spec impact (additive minor): new normative §Dry-run section in `cli-contract.md` defining the per-verb opt-in contract for `-n` / `--dry-run`; verb catalog rows for `sm init`, `sm db reset` (default + `--state` + `--hard`), and `sm db restore` amended to declare and describe their `--dry-run` previews; exit-code `2` row clarified to mention runtime / environment mismatches explicitly; `sm scan --compare-with <path>` flag replaced by `sm scan compare-with <dump> [roots...]` sub-verb (pre-1.0 breaking shipped as minor per `versioning.md` § Pre-1.0). CLI impact (minor): `ExitCode` enum exported, `ScanCompareCommand` registered, `IPluginLoaderOptions.loadTimeoutMs` (default 5000ms) added; `runScanInternal` decomposed into a 89-line composer + 4 pure functions (`validateRoots` / `indexPriorSnapshot` / `walkAndDetect` / `runRules`); `withSqlite` / `tryWithSqlite` helpers consolidate the open / use / close idiom across 20 call sites and fold scan's read-prior + persist double-open into one; `--strict --json` self-validates the produced `ScanResult` against `scan-result.schema.json`; external-link discrimination promoted to RFC 3986 scheme regex; prior snapshot validated under `--strict` in scan + watch; per-extension error reporting via `extension.error` ProgressEvent + CLI stderr renderer; type naming convention documented in `kernel/types.ts` + AGENTS.md (no rename); error / hint strings extracted to `*.texts.ts` modules with `{{name}}` template interpolation via the new `kernel/util/tx.ts` helper. Bonus (not a finding): `001_initial.sql` + `002_scan_meta.sql` consolidated into single `001_initial.sql` (pre-1.0, no released DBs to forward-migrate). `spec/index.json` regenerated (58 files hashed); `npm run spec:check` green. **33 new tests** (2 plugin-loader timeout, 3 orchestrator extension errors, 4 CLI emitter wiring, 5 init dry-run, 9 db dry-run, 13 `tx` helper); test totals 473 → **512 of 512 pass** (482 cli + 30 testkit). 1 changeset pending: `.changeset/audit-pre-1.0.md` (minor for spec + minor for cli — bundled review-pass unit). **Earlier prose**: **Step 9 fully closed** across four sub-steps (9.1 — plugin runtime wiring; 9.2 — plugin migrations + three-layer prefix-enforcement rule + `sm db migrate --kernel-only` / `--plugin <id>`; 9.3 — `@skill-map/testkit` published as a third public package; 9.4 — plugin author guide at `spec/plugin-author-guide.md`, reference plugin at `examples/hello-world/`, diagnostics polish on `PluginLoader.reason` strings). 9.4 adds **prose-only spec content** (additive `spec/plugin-author-guide.md`; `spec/index.json` regenerated 57 → 58 hashed files; `spec/package.json#files` updated; `coverage.md` unchanged because the guide is prose, not a schema). Reference plugin is the smallest viable shape: one detector emitting `references` links per `@greet:<name>` token, intentionally trivial, verified end-to-end (loads under `sm plugins list`, contributes to `sm scan`, tests pass via `node --test`). Diagnostics polish: each `PluginLoader.reason` failure mode now carries an actionable hint — invalid-manifest names the manifest path + points at `plugins-registry.schema.json#/$defs/PluginManifest`; incompatible-spec suggests two remediations (update plugin's specCompat, or pin sm to a compatible spec); load-error (file not found) includes the absolute resolved path; load-error (unknown / missing kind) lists the valid kinds; load-error (kind schema fails) names `spec/schemas/extensions/<kind>.schema.json`. **6 new tests** under `test/plugin-loader.test.ts` (`Step 9.4 diagnostics polish` describe block) assert each hint shape is present without pinning the full text. **Step 9 totals**: 4 sub-steps; 84 new tests (5 in 9.1 + 43 in 9.2 + 30 in 9.3 + 6 in 9.4); 4 pending changesets in `.changeset/` (`step-9-1-plugin-runtime-wiring.md`, `step-9-2-plugin-migrations.md`, `step-9-3-testkit.md`, `step-9-4-plugin-author-docs.md`). Total project tests 389 → **473 pass** (443 cli + 30 testkit). Plugins moved from "discovered but inert" to a first-class authoring surface with docs, tests, and a working reference. **Next step (post-pivot 2026-05-02 — Decision #118)**: **Step 14 — Full Web UI** (sub-stepped 14.1–14.7, ships v0.6.0). Step 10 paused with Phase 0 dormant; Phases A–G resume after Step 14 closes and ship v0.8.0.
>
...
- ✅ **`external-url-counter` detector** landed as the 4th detector — one new file under `src/extensions/detectors/external-url-counter/` + one entry in `built-ins.ts`. Validates Step 2's drop-in litmus. Emits pseudo-links the orchestrator partitions into `node.externalRefsCount` (never persisted to `scan_links`, never reach rules).
...
- Sub-step 7.2 ✅ — detector conflict resolution. Two pieces. **(a)** New built-in rule `link-conflict` (`src/extensions/rules/link-conflict/`): groups `scan_links` rows by `(source, target)` and emits one `warn` Issue per pair where the set of distinct `kind` values has size ≥ 2; `data` carries `{ source, target, variants: [{ kind, sources, confidence }, ...] }`. Cross-detector AGREEMENT (single kind across multiple detectors) is silent by design — confirming the happy path would generate massive noise on real graphs. Severity is `warn`, not `error`: the rule cannot pick which kind is correct, so per `cli-contract.md` §Exit codes the verb stays exit 0. **(b)** `sm show` pretty link aggregation: human renderer now groups `linksOut` / `linksIn` by `(endpoint, kind, normalizedTrigger)` and prints one row per group with the union of detector ids in a `sources:` field; section header reports raw + unique counts (`Links out (12, 9 unique)`); `(×N)` suffix when N detectors emit the same logical link. `--json` output stays raw rows (Decision #90 untouched — storage keeps one row per detector). UI inspector aggregation explicitly **deferred to Step 14**: the current Flavor A renders `metadata.{related, requires, supersedes, provides, conflictsWith}` chips directly from the frontmatter, not from `scan_links`; when Flavor B lands (Hono BFF + WS + full link panel from scan), the aggregation logic from `src/cli/commands/show.ts` will need to be ported.
...
- **8.3 — `sm export <query> --format <json|md|mermaid>`** ✅ — replaces the stub. Mini query language (whitespace-separated `key=value`, AND across keys, comma-separated values OR within a key): `kind` (skill / agent / command / hook / note), `has` (`issues` today; `findings` / `summary` reserved for Steps 10 / 11), `path` (POSIX glob with `*` / `**`). New kernel module `src/kernel/scan/query.ts` exports `parseExportQuery` + `applyExportQuery` + `IExportQuery` + `IExportSubset` + `ExportQueryError` (pure, no IO; zero-dep micro-glob → RegExp). Subset semantics: nodes pass under AND-of-filters; links require BOTH endpoints in scope (closed subgraph — boundary edges would confuse focused-view with focused-and-neighbours); issues survive when ANY of their `nodeIds` is in scope (cross-cutting issues like `trigger-collision` stay visible). Formats `json` and `md` real today; `mermaid` exits 5 with a Step-12 pointer (renderer plug-in lands as a built-in there). Exit 5 on bad format / bad query / missing DB. Step 8 fully closed.
...
- **Phase G — Conformance, Skill agent, events, polish**. New conformance case `extension-mode-derivation` (an audit composing one deterministic + one probabilistic primitive resolves to effective mode `probabilistic`). `/skill-map:run-queue` + `sm-cli-run-queue` Skill agent package. Job event emission per `spec/job-events.md` (`run.*`, `job.*`, `model.*`, `run.reap.*`). `github-enrichment` bundled plugin (hash verification). ROADMAP + `coverage.md` updated.
