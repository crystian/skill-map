...
**Last updated**: 2026-04-26 (added Decision #89: `llms.txt` + context7 surface for AI-agent discoverability, scheduled at Step 14). Changes land via `.changeset/*.md` and `spec/CHANGELOG.md` — this header stops paraphrasing them.
...
**Status**: Steps **0a**, **0b**, **0c**, **1a**, **1b**, **1c**, **2**, **3**, and **4** are **complete**. `@skill-map/spec` is published on npm; the `ui/` workspace ships Flavor A. Step 4 shipped the scan end-to-end pipeline: `sm scan` persists ScanResult into `<scope>/.skill-map/skill-map.db` with replace-all transactional snapshots and auto-migrate; per-node token counts via `js-tiktoken` cl100k_base with `--no-tokens` opt-out; `external-url-counter` detector (4th detector, drop-in proof from Step 2) emitting pseudo-links the orchestrator partitions into `external_refs_count` (never persisted to `scan_links`); `sm scan -n / --dry-run` (skips all DB writes) and `--changed` (incremental — reuses unchanged nodes by `bodyHash + frontmatterHash` match, degrades to full scan when no prior snapshot exists); `sm list / show / check` reading from `scan_*` (replaced their stubs); self-scan acceptance test validating the live repo against `scan-result.schema.json`; 500-MD performance benchmark (~1037 ms vs 2000 ms budget). Sub-step 4.7 reconciled the runtime ScanResult with the spec — runtime caught up (`scannedAt` int, `scope`, `adapters`, `scannedBy`, `stats.filesWalked / filesSkipped`); spec untouched, was authoritative all along. Sub-step 4.8 closed three bugs surfaced by an end-to-end manual validation pass: `sm scan` exit code now matches the spec (1 only when issues at `error` severity exist, was 1 for any issue), `sm show` human output now reports `External refs: <N>` after the Weight section, and `sm scan --changed` no longer drops `supersedes`-inversion links from cached nodes (`originatingNodeOf` heuristic over the existing schema — no migration). Bug fixed in `defaultMigrationsDir()` (latent regression in dist bundle layout). 127 of 127 tests pass. Next step: **Step 5 — History + orphan reconciliation**. The canonical completeness marker lives in §Execution plan below.
