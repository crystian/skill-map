...
[![spec](https://img.shields.io/badge/spec-v0.11.0-8A2BE2)](./spec/)
[![impl](https://img.shields.io/badge/impl-v0.9.0-5D3FD3)](./src/)
...
**Estado**: los Steps **0a–9 están completos** (spec, kernel, plugin loader, superficie CLI completa, UX para autores de plugins). **Step 14 — Full Web UI** está en progreso: los sub-steps 14.1 (esqueleto Hono BFF), 14.2 (endpoints REST read-side), 14.3 (modos live + demo) y 14.4 (broadcaster WebSocket + UI reactiva) ya shippearon; 14.5–14.7 (pulido + bundle budgets + scope responsive) siguen pendientes. Shipea **v0.6.0 — kernel determinista + CLI + Web UI** (per Decision #118). La wave 2 (subsistema de jobs + extensiones probabilísticas) retoma cuando Step 14 cierre y shipea v0.8.0. Los paquetes npm `@skill-map/spec` y `@skill-map/cli` están publicados (versiones en `spec/package.json` y `src/package.json`); `@skill-map/testkit` se sumó como tercer paquete público. Ver [ROADMAP.md](./ROADMAP.md) para el marcador de completitud y el plan de ejecución completo.
