...
- f8fca25: Step 10 prep — job artifacts move into the database (B2: content-addressed storage)
  ...
  **Schema-typed contract changes**
  ...
  - `Job.failureReason` and `ExecutionRecord.failureReason` enum **semantics shifted, name preserved**: `job-file-missing` no longer means "missing on-disk job file"; it now flags a missing `state_job_contents` row (DB-corruption-only state — the runtime invariant keeps this state unreachable, since submit and prune both hold the two tables consistent). The legacy enum NAME is retained across the disk-to-DB shift to keep the failure-reason vocabulary backward-compatible per `job-lifecycle.md §Atomicity edge cases`. No schema changes to enum values.
  - `history-stats.schema.json` `perFailureReason` enum unchanged; the per-key counters now bucket the new semantics under the same `job-file-missing` label.
