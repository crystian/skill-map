...
First publish of the alias / placeholder packages, all under `alias/*`. Each one is functionally inert: the bin (`<name>` and `sm`) prints a warning to stderr pointing at `@skill-map/cli` and exits with code 1.
