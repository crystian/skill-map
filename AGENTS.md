...
- **Never bump versions manually** — every PR that touches a workspace (`spec/` and `src/` today, `ui/` later) ships a `.changeset/*.md` (`npm run release:changeset`). The release workflow opens a "Version Packages" PR; merging it bumps versions and publishes. See `CONTRIBUTING.md`.
